...
# ── Resident/Delivery Extension Modules ──────────────────────────────────────────
from db import init_db
from resident_routes import router as resident_router, create_default_operator_if_empty
...
# ── Ensure upload and static directories exist (must be before lifespan) ───────────
UPLOAD_DIR = "uploads"
...
class RobotState:
    def __init__(self):
        ...
        # Poll guard — prevents status poll tasks from piling up under lock contention
        ...
        # ESP32-CAM Hardware Module Telemetry
        self.esp32cam_connected = False
        self.camera_frame = ""
        self.esp32cam_ip = "Disconnected"
        self.esp32cam_fps = 0.0
        self.esp32cam_resolution = "VGA (640x480)"
        self.esp32cam_flash_led = False
...
async def lifespan(app: FastAPI):
    ...
    # ── Initialise SQLite database (resident/delivery system) ────────────────────
    try:
        init_db()
        create_default_operator_if_empty()
    ...
    # Initialise the last-success timestamp to NOW so the first 15 seconds of runtime
    # never cause a false disconnect even if the first /command call fails.
    global _esp32_last_cmd_success_time
    _esp32_last_cmd_success_time = time.time()
    print("[Startup] ESP32 last-success timestamp initialised to now.")
...
# ── Mount resident/delivery/auth router ──────────────────────────────────────────
# All new endpoints (/auth/*, /me, /deliveries, /operator/*, /ws/resident) are
# added here. Existing robot/telemetry endpoints are completely unchanged.
...
# Initialise to time.time() so fresh backend restarts get a 15-second grace
# period before any failed /command call can flip esp32_connected to False.
_esp32_last_cmd_success_time: float = 0.0   # set to current time at startup
...
# Motor wiring inversion remap.
# The physical motors are wired in reverse, so every logical direction must be
# flipped before sending to the ESP32.  Applying this once here means ALL callers
# (manual joystick, auto-navigation, obstacle-avoidance) are corrected uniformly.
# Manual endpoint no longer needs its own remap.
_DIRECTION_REMAP = {
    "FORWARD":  "BACKWARD",
    "BACKWARD": "FORWARD",
    "LEFT":     "RIGHT",
...
async def send_esp32_command(endpoint: str, params: dict = None) -> bool:
    ...
    if endpoint == "/command" and params and "direction" in params:
...
def generate_auto_mission_actions():
    ...
    # Start with no assumed heading so the robot just drives toward the first segment
    # without an unwanted pre-turn. current_heading is updated after each segment.
    ...
    for i in range(len(path) - 1):
        ...
        for j in range(len(conn_path) - 1):
            ...
            # Handle turnings.
            # In screen/image coordinates, Y increases downwards:
            #   diff > 0 → the turn is counterclockwise on screen → physical RIGHT (clockwise when
            #              viewed from above with screen-right = robot-right and screen-down = forward)
            #   diff < 0 → the turn is clockwise on screen → physical LEFT.
            # Because the map Y-axis is inverted relative to standard math, a POSITIVE diff
            # means the heading rotated clockwise on screen which the robot executes as a RIGHT turn.
...
# Runs every 5 s when not navigating, skips entirely when mission poll is active.
# Requires 3 consecutive failures before marking disconnected (avoids false drops
# during brief ESP32 busy periods such as gyro calibration on /start).
# Also respects _esp32_last_cmd_success_time: if a real command succeeded in the
# last 15 s we never flip to disconnected — the health-check status is advisory only.
async def esp32_health_check_loop():
    ...
    while True:
        try:
            # Skip health check when a mission poll is already running or navigating —
            # poll_esp32_status_during_mission() already updates esp32_connected at 5 Hz
            if robot.status == "MOVING" or robot._poll_running:
                consecutive_failures = 0  # Reset on skip so mission-phase doesn't inherit stale count
                await asyncio.sleep(5)
                continue

            # If a real command succeeded very recently, the ESP32 is clearly reachable.
            # Skip this ping cycle and keep the connected flag as-is.
            if time.time() - _esp32_last_cmd_success_time < 10.0:
                robot.esp32_connected = True
                consecutive_failures = 0
                await asyncio.sleep(5)
                continue
            ...
            attempt_number += 1
            # Use a fresh client each time to avoid stale keep-alive on ESP32 AP WiFi.
            # 4 s timeout: long enough to survive gyro calibration on /start (~3 s).
            async with httpx.AsyncClient(
                ...
                if resp.status_code == 200:
                    robot.esp32_connected = True
                    consecutive_failures = 0
                    _esp32_last_cmd_success_time = time.time()  # Treat a successful ping as a cmd success
                    robot.last_api_response = f"ESP32 PING OK (HTTP {resp.status_code})"
                    print(f"[HealthCheck] ESP32 reachable at {robot.esp32_ip} — OK")
                    try:
                        data = resp.json()
                        raw_d = data.get("distance", 999)
                        # Sanitise: -1 (in-flight sentinel) and 0 are invalid — treat as clear
                        safe_d = raw_d if raw_d > 0 else 999
                        robot.distance   = safe_d
                        ...
                        # Update obstacle flag with the same valid-range guard
                        robot.obstacle_detected = (1 <= safe_d <= 20)
                    except Exception:
                        pass  # Non-JSON response is still a valid ping

                else:
                    consecutive_failures += 1
                    print(f"[HealthCheck] Ping #{attempt_number} to {robot.esp32_ip} — HTTP {resp.status_code} (failure {consecutive_failures})")
                    if consecutive_failures >= 3 and time.time() - _esp32_last_cmd_success_time > 15.0:
                        robot.esp32_connected = False
                        robot.last_api_response = f"ESP32 UNREACHABLE — HTTP {resp.status_code}"

        except Exception as e:
            consecutive_failures += 1
            err_msg = f"{type(e).__name__}: {e}" if str(e) else type(e).__name__
            print(f"[HealthCheck] Ping #{attempt_number} to {robot.esp32_ip} FAILED ({consecutive_failures} consecutive): {err_msg}")
            # Only mark offline after 3 consecutive failures AND no recent successful command
            if consecutive_failures >= 3 and time.time() - _esp32_last_cmd_success_time > 15.0:
                robot.esp32_connected = False
                robot.last_api_response = f"ESP32 UNREACHABLE — {err_msg}"
...
async def poll_esp32_status_during_mission():
    # Guard: skip if a previous poll is still in flight (prevents pile-up under lock)
    if robot._poll_running:
        return
    robot._poll_running = True
    url = f"http://{robot.esp32_ip}/status"
    client = await get_esp32_client()
    try:
        async with esp32_lock:
            ...
            if resp.status_code == 200:
                data = resp.json()
                raw_dist = data.get("distance", 999)
                # Sanitise: -1 = echo in-flight (should not appear after firmware fix
                # but guard here too for old firmware compatibility).  Also reject 0.
                dist = raw_dist if raw_dist > 0 else 999
                robot.esp32_connected = True
                ...
                # Check for obstacle: only valid positive readings <= 20 cm count.
                # Sentinel values (-1, 0) and 999 (clear/timeout) must never
                # trigger obstacle_detected to avoid false alarms on connect.
                if 1 <= dist <= 20:
                    robot.obstacle_detected = True
                else:
                    robot.obstacle_detected = False
...
async def update_auto_navigation():
    ...
    # Guard: if start_sequence() hasn't finished setting action_start_time yet,
    # skip this cycle to avoid a premature STOP command being sent.
    if robot.action_start_time == 0:
        return
    ...
    if now - robot.last_status_poll_time >= poll_interval:
        ...
        # Only create a new poll task if no poll is currently in flight
        if not robot._poll_running:
...
def get_telemetry_payload():
    return {
        ...
        # ESP32-CAM Hardware Module Telemetry
        "esp32cam_connected": getattr(robot, "esp32cam_connected", False),
        "camera_frame": getattr(robot, "camera_frame", ""),
        "esp32cam_ip": getattr(robot, "esp32cam_ip", "Disconnected"),
        "esp32cam_fps": getattr(robot, "esp32cam_fps", 0.0),
        "esp32cam_resolution": getattr(robot, "esp32cam_resolution", "VGA (640x480)"),
        "esp32cam_flash_led": getattr(robot, "esp32cam_flash_led", False),
        "esp32cam_rssi": getattr(robot, "esp32cam_rssi", -60),
...
async def start_robot():
    ...
    # Prepare ESP32 for backend-controlled auto navigation.
    # IMPORTANT: We do NOT call /start (which would enable the ESP32's LDE/mission mode).
    # In mission mode the ESP32 drives itself autonomously AND the backend sends /command
    # calls — the two control systems conflict, causing direction inversions.
    # Keeping the ESP32 in "auto" mode means it executes every /command exactly like
    # manual mode, which is already confirmed to work correctly.
    async def start_sequence():
        # Mark MOVING immediately so the health-check loop skips its ping cycle
        # during setup.  The auto-nav loop won't act on this until action_start_time
        # is set below, because current_action_idx starts at 0 and elapsed will be 0.
        robot.status = "MOVING"
        ...
        # endpoint.  Failure here is non-fatal — the heading PID will self-correct.
        try:
            async with httpx.AsyncClient(timeout=httpx.Timeout(1.0, connect=0.5)) as _rc:
                r = await _rc.get(f"http://{robot.esp32_ip}/reset-yaw")
                if r.status_code == 200:
                    print("[StartSeq] Yaw reset OK")
                else:
                    print(f"[StartSeq] /reset-yaw returned {r.status_code} — continuing without reset")
        except Exception as _e:
            print(f"[StartSeq] /reset-yaw unavailable ({_e}) — continuing without reset (old firmware?)")

        print("Auto-nav ready (ESP32 in auto mode). Navigation loop will drive the robot.")
        robot.action_start_time = asyncio.get_event_loop().time()  # Reset timer for first action
...
async def set_command(direction: str):
    ...
    # Build the ESP32 command params.
    # LEFT/RIGHT: send direction + speed ONLY — do NOT include a duration.
    # Sending a duration causes the ESP32 to set running=True and robotMode="auto"
    # (auto-timer path), which conflicts with the gyro PID pendingTurn queue and
    # causes turns to be ignored or to stall mid-pivot.
    # The gyro PID turn is self-terminating (stops once the angle target is hit),
    # so no external timer is needed for manual turn commands.
    # NOTE: direction remap (motor inversion correction) is applied centrally
    # inside send_esp32_command — no remap needed here.
    if direction in ("LEFT", "RIGHT"):
        ...
        # FORWARD / BACKWARD / STOP — no fixed duration (continuous hold)
        esp32_params = {"direction": direction}

    # Forward command to ESP32 (remap applied inside send_esp32_command)
    asyncio.create_task(send_esp32_command("/command", esp32_params))
...
# ─────────────────────────────────────────────────────────────────────────────
# ESP32-CAM Hardware Stream Receiver (WebSocket & HTTP endpoints)
# ─────────────────────────────────────────────────────────────────────────────
@app.websocket("/ws/esp32cam-stream")
@app.websocket("/ws/robot-stream")
async def websocket_esp32cam_stream(websocket: WebSocket):
    await websocket.accept()
    robot.esp32cam_connected = True
    client_host = websocket.client.host if websocket.client else "ESP32-CAM"
    robot.esp32cam_ip = client_host
    print(f"[ESP32-CAM] Hardware Camera Module Connected from {client_host}!")
    try:
        await websocket.send_json({"action": "START_CAMERA", "status": "OK"})
    except Exception:
        pass
        ...
        while True:
            ...
            if "bytes" in message and message["bytes"]:
                ...
                if now - last_frame_time >= 1.0:
                    robot.esp32cam_fps = round(frame_count / (now - last_frame_time), 1)
                    frame_count = 0
                    last_frame_time = now
                ...
                await manager.broadcast({
                    ...
                    "frame": frame_b64,
                    "timestamp": time.time(),
                    "esp32cam_fps": robot.esp32cam_fps,
                })
                ...
                try:
                    ...
                    if msg_type in ("telemetry", "status"):
                        if "ip" in data:
                            robot.esp32cam_ip = data["ip"]
                        if "rssi" in data:
                            robot.esp32cam_rssi = data["rssi"]
                        if "resolution" in data:
                            robot.esp32cam_resolution = data["resolution"]
                        if "flash_led" in data:
                            robot.esp32cam_flash_led = bool(data["flash_led"])
                    elif msg_type == "image":
                        ...
                        if frame_b64:
                            ...
                            if (VISION_AVAILABLE and robot.vision_enabled
                                    and not robot._vision_processing):
                                robot._vision_processing = True
                                asyncio.create_task(_process_vision_frame(frame_b64))
                except Exception:
                    pass
    except WebSocketDisconnect:
        print("[ESP32-CAM] Hardware Camera Module Disconnected.")
        robot.esp32cam_connected = False
    except Exception as e:
        print(f"[ESP32-CAM] Stream error: {e}")
        robot.esp32cam_connected = False

@app.post("/api/esp32cam/upload")
async def esp32cam_upload_frame(request: Request):
    """HTTP POST endpoint for ESP32-CAM firmware pushing JPEG image frames."""
    try:
        body = await request.body()
        if body:
            frame_b64 = base64.b64encode(body).decode('utf-8')
            robot.camera_frame = frame_b64
            robot.esp32cam_connected = True
            robot.esp32cam_ip = request.client.host if request.client else "ESP32-CAM"
...
@app.get("/api/esp32cam/control")
async def esp32cam_proxy_control(var: str, val: int, ip: Optional[str] = None):
    """Proxy OV2640 camera settings commands from dashboard to ESP32-CAM."""
    target_ip = ip or robot.esp32cam_ip
    if not target_ip or target_ip == "Disconnected":
        return {"status": "error", "message": "ESP32-CAM IP not specified or disconnected"}
    
    url = f"http://{target_ip}/control?var={var}&val={val}"
    try:
        async with httpx.AsyncClient(timeout=3.0) as client:
            resp = await client.get(url)
            if resp.status_code == 200:
                robot.esp32cam_connected = True
                return {"status": "success", "var": var, "val": val}
