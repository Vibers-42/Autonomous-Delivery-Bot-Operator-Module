...
class _MissionPlannerScreenState extends State<MissionPlannerScreen> {
  ...
  Widget _buildFreeRoamPanel(BuildContext context, RobotStateProvider provider) {
    ...
    final bool obstacleNear = distCm >= 1.0 && distCm <= 20.0;
        ...
        children: [
          ...
          Row(
            children: [
              ...
              const Spacer(),
              Text(
                (distCm <= 0 || distCm >= 990) ? "CLEAR" : "${distCm.toStringAsFixed(0)} cm",
                style: TextStyle(
  ...
  Widget _buildDistanceSensorBar(double distCm) {
    // If clear or invalid (>400 or <=0 or 999), show empty bar (fraction = 0)
    final bool isClear = distCm <= 0 || distCm >= 990;
    final double capped = isClear ? 150.0 : distCm.clamp(0, 150);
    final double fraction = isClear ? 0.0 : (1.0 - (capped / 150.0)); // 0 = far/clear, 1 = very close
    Color barColor;
    if (isClear) {
  ...
  Widget _buildMissionMonitor(BuildContext context, RobotStateProvider provider) {
    ...
    } else if (provider.distanceSensor >= 1.0 && provider.distanceSensor <= 20.0) {
