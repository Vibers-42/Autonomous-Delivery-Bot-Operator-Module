...
class RobotStateProvider extends ChangeNotifier {
  ...
  bool _esp32CamConnected = false;
  String _cameraFrame = "";
  String _esp32CamIp = "Disconnected";
  double _esp32CamFps = 0.0;
  String _esp32CamResolution = "VGA (640x480)";
  bool _esp32CamFlashLed = false;
  int _esp32CamRssi = -60;

  bool get esp32CamConnected => _esp32CamConnected;
  String get cameraFrame => _cameraFrame;
  String get esp32CamIp => _esp32CamIp;
  double get esp32CamFps => _esp32CamFps;
  String get esp32CamResolution => _esp32CamResolution;
  bool get esp32CamFlashLed => _esp32CamFlashLed;
  int get esp32CamRssi => _esp32CamRssi;

  // Backward compatibility getters
  bool get phoneConnected => _esp32CamConnected;
  Map<String, dynamic> get gps => {'lat': 0.0, 'lon': 0.0, 'speed': 0.0, 'heading': 0.0, 'accuracy': 0.0};
  Map<String, dynamic> get imu => {'roll': 0.0, 'pitch': 0.0, 'yaw': _yaw};
  Map<String, dynamic> get phoneBattery => {'level': 100, 'charging': false};
  Map<String, dynamic> get phoneStatus => {'wifi_signal': _esp32CamRssi, 'avail_mem_mb': 4, 'storage_free_gb': 0.5};
  ...
  RobotStateProvider() {
    // All robot control commands (start/stop/command/mode/speed) go through the
    // FastAPI backend at _backendIp — the backend relays them to the ESP32.
    // Do NOT point _robotController at the ESP32 IP directly: the laptop is not
    // connected to the ESP32 WiFi AP, so direct calls would always time out and
    // the backend health-check would never see traffic → shows Disconnected.
  ...
  void updateSettings({
    ...
    // Robot controller and backend service both target the FastAPI backend.
    // The backend relays all commands to the ESP32 via its own HTTP client.
    _robotController.updateBaseUrl(_backendIp);
    _backendService.updateBaseUrl(_backendIp);
  ...
  void connectTelemetry({bool silent = false}) {
    ...
    _telemetrySubscription = telemetryStream.listen(
      (data) {
        ...
        final num? rawDist = data["distance"] as num?;
        if (rawDist != null) {
          final double d = rawDist.toDouble();
          _distanceSensor = (d <= 0 || d > 400) ? 999.0 : d;
        } else {
          _distanceSensor = 999.0;
        }
        ...
        // Parse ESP32-CAM Hardware Module Telemetry
        _esp32CamConnected = data['esp32cam_connected'] ?? false;
        _esp32CamIp = data['esp32cam_ip'] ?? "Disconnected";
        _esp32CamFps = (data['esp32cam_fps'] as num?)?.toDouble() ?? 0.0;
        _esp32CamResolution = data['esp32cam_resolution'] ?? "VGA (640x480)";
        _esp32CamFlashLed = data['esp32cam_flash_led'] ?? false;
        _esp32CamRssi = (data['esp32cam_rssi'] as num?)?.toInt() ?? -60;
        ...
        final bool obstacleDetected = _distanceSensor >= 1.0 && _distanceSensor <= 20.0;
  ...
  // In-flight guard — prevents concurrent manual command HTTP calls from piling up
  // when the user taps buttons rapidly.
  bool _manualCmdInFlight = false;

  // Manual driving commands (FORWARD, BACKWARD, LEFT, RIGHT, STOP)
  //
  // IMPORTANT: do NOT call switchMode() here. The mode switch is handled once by
  // the Manual Override toggle in the UI (switchMode called from that switch widget).
  // Calling switchMode() before every button press blocks for 2-4 seconds
  // (HTTP round-trip to backend → ESP32 /mode → ESP32 /stop) — this was the root
  // cause of the delayed/non-responsive movement.
  Future<void> triggerManualCommand(String direction) async {
    // Drop duplicate in-flight commands to prevent queue pile-up
    if (_manualCmdInFlight) return;
    _manualCmdInFlight = true;

    // Optimistically update the UI immediately so the button highlights instantly
